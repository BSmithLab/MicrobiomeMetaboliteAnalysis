...
wd2 = "C:/Users/jeshima/Documents/Smith Lab/Spring 2023/BMC Microbiology Paper/Data/EF"
setwd(wd2)
...
if(combinebroadpeaks == T){
  wd = "C:/Users/jeshima/Documents/Smith Lab/Spring 2023/BMC Microbiology Paper/Data/EF/RTs"
  setwd(wd)
...
Remove_LowRT = c(0,2.04,2.3,2.38,2.937,3.567,4.51,4.95,6.01,7.47,7.95,8.235,8.72,9.3,12.02) #Not needed, if ClearNoise = False
Remove_HighRT = c(1.94,2.18,2.37,2.884,3.085,3.772,4.88,5.21,6.64,7.475,7.97,8.52,8.74,9.748,15.6) #Not needed, if ClearNoise = False
...
OD600_pos = c(0.080,0.089,0.080)
OD600_neg = c(0.097,0.101,0.085)
...
sigthresh = 0.2 #Significance threshold
  ...
  if(length(Cind) >= 3){ 
    ...
    if(length(Cind) >= 3){ 
      ...
      for(i in 1:nrow(plotdata)){
        ...
        #Dataframe for plotting
        df$Condition = factor(df$Condition,levels=c("+L-Dopa","-L-Dopa","BHI Control","AFMT Inhibition"))
...
if(RunPCA == T){
  ...
  if(length(conditions) <= 3){
    ...
    if(pca3d == T){
      ...
      plot(PC1,PC2,xlim=c((min1 + (0.2*min1)),(max1 + (0.6*max1))),ylim=c((min2 + (0.2*min2)),(max2 + (0.6*max2))),main=paste("Unsupervised PCA for ",organism,sep=""),col=pcacolors,pch=20,cex=2,cex.lab = 1.5,cex.main = 1.5,cex.axis = 1.5)
      legend(max1 + (0.13*max1),max2 + (0.55*max2),legend = c("Positive","Negative","Control","AFMT Inhibition"),col = c(pcacolorpos,pcacolorneg,pcacolorctr,pcacolorex),pch=20,pt.cex = 1.5)
    }
