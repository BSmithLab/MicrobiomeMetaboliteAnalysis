...
Remove_HighRT = 15.6 #Not needed, if ClearNoise = False
...
#If useIS is TRUE:
IS_Abundance_pos = c(939371,681555,685939)
...
heatcolorlow = "turquoise4"
heatcolormid = "white"
heatcolorhigh = "maroon3"
limits = c(-3,3)
clusterrows = T
clustercols = F
  ...
  if(length(Cind) >= 3){ 
    ...
    if(length(Cind) >= 3){ 
      ...
      for(i in 1:nrow(plotdata)){
        ...
        df = data.frame(matrix(NA,length(conditions),3))
        colnames(df) = c("Condition","Average","SE")
        df$Condition = c("+L-Dopa","-L-Dopa","BHI Control","Tungstate Inhibition")
        ...
        #Dataframe for plotting
        df$Condition = factor(df$Condition,levels=c("+L-Dopa","-L-Dopa","BHI Control","Tungstate Inhibition"))
      ...
      for(i in 1:nrow(plotdata)){
        ...
        if(length(Cind) >= 1){
          ...
          #Dataframe for plotting
          df$Condition = factor(df$Condition,levels=c("BHI Control","-L-Dopa","+L-Dopa","Tungstate Inhibition"))
          colors = c(ctrcolor,negcolor,poscolor,excolor)
          
        }else{
          df = data.frame(matrix(NA,length(conditions),3))
          colnames(df) = c("Condition","Average","SE")
          df$Condition = c("+L-Dopa","-L-Dopa","Tungstate Inhibition")
          ...
          #Dataframe for plotting
          df$Condition = factor(df$Condition,levels=c("-L-Dopa","+L-Dopa","Tungstate Inhibition"))
