...
#v.6.8.1 - Fixed p.adjust bug (adjust happened pairwise instead of considering all hypotheses tested)
...
#META_SmithLab_GCMS_AutomatedDataProcessingandStats_v1-1-0.R
...
#6 - version 6-9-0 and later) FiltSilox() if useupgrade = T
...
wd2 = "D:/Taylor/BMC Microbiology Paper/Data/CS"
setwd(wd2)
...
if(combinebroadpeaks == T){
  wd = "D:/Taylor/BMC Microbiology Paper/Data/CS/RTs"
  setwd(wd)
...
#Use upgraded filering function (FiltSilox from UsefulFunctions_v2)
useupgrade = F
...
Remove_LowRT = c(0,1.56,1.715,1.865,2.033,2.38,2.68,3.314,4.491,6.11,6.251,6.63,7.36,7.47,7.835,8.36,8.48,8.59,8.67,9.51,9.87,9.911,10.05,10.23,10.35,10.86,11.13,11.4,11.7,12.025,12.14,12.185,12.5) #Not needed, if ClearNoise = False
Remove_HighRT = c(1.41,1.67,1.83,1.914,2.325,2.616,3.22,4.38,5.14,6.17,6.34,6.87,7.39,7.69,8.23,8.41,8.53,8.599,8.7,9.7,9.888,9.92,10.12,10.31,10.8,11.04,11.32,11.5,11.82,12.03,12.15,12.19,15.7) #Not needed, if ClearNoise = False
  ...
  if(FilterBefore == T){
    ...
    if(combinebroadpeaks == T){
      ...
      for(j in 1:nrow(RTind)){
        ...
        index1 = which(as.numeric(cleandata$RT) >= RTind$Start[j])
        index2 = which(as.numeric(cleandata$RT) <= RTind$Stop[j])
...
if(removecontam == T){
  ...
  if(useupgrade == T){
    FiltSilox(FinalData,userownames = T,dioxolanes = T,oxalics = T)
  }
}
