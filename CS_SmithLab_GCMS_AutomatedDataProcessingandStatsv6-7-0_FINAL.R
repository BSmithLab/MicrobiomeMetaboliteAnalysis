...
#V7.1 - Allows user to specify parametric (student's t-test) OR nonparametric (Wilcoxon rank sum) statistical test
#V7.2 - Regression, nonparametric statistics, and Bayesian/Gibbs/bootstrapping functionality added.
...
wd2 = "C:/Users/jeshima/Documents/Smith Lab/Spring 2023/BMC Microbiology Paper/Data/CS"
setwd(wd2)
...
conditions = c("Pos","Neg","Ctr") #UNIQUE FILE HANDLE MUST BE EXACTLY 3 CHARACTERS LONG
...
if(combinebroadpeaks == T){
  wd = "C:/Users/jeshima/Documents/Smith Lab/Spring 2023/BMC Microbiology Paper/Data/CS/RTs"
  setwd(wd)
  RTind = read.csv("CS_RTs.csv")
}
...
Remove_LowRT = 13.1 #Not needed, if ClearNoise = False
Remove_HighRT = 15.7 #Not needed, if ClearNoise = False
...
#If useIS is TRUE:
IS_Abundance_pos = c(1740341,8979540,5385005)
IS_Abundance_neg = c(5097434,8724907,6515175)
IS_Abundance_ctr = c(1714161,518265,906757)
...
#Barplot Colors:
poscolor = "darkviolet"
negcolor = "darkseagreen4"
ctrcolor = "grey60"
excolor = "tan1" #This is only needed if 4 conditions are provided
...
heatcolorlow = "darkolivegreen"
heatcolormid = "white"
heatcolorhigh = "purple4"
limits = c(-3,3)
clusterrows = T
clustercols = F
...
pcacolorpos = "darkviolet"
pcacolorneg = "darkolivegreen"
pcacolorctr = "grey71"
pcacolorex = "darkorange2" #This is only needed if 4 conditions are provided
...
if(FilterUsingControls == T){
  ...
  FinalData = FinalData[! round(FinalData$RT,2) %in% round(FilterRTs,2),]
  
  if(removecontrols == T){
    FinalData = FinalData[,-keep]
  }
  ...
  if(length(Cind) >= 3){ 
    ...
    for(i in 1:nrow(plotdata)){
      ...
      if(length(conditions) == 2){
        ...
        df$Condition = factor(df$Condition,levels=c("+L-Dopa","-L-Dopa","BHI Control"))
...
if(RunPCA == T){
  ...
  if(length(conditions) <= 3){
    ...
    if(pca3d == T){
      ...
      legend(max1 + (0.2*max1),max2 + (0.55*max2),legend = c("Positive","Negative","Control"),col = c(pcacolorpos,pcacolorneg,pcacolorctr),pch=20,pt.cex = 1.5)
    }else{
