...
#7) Save that sheet as a .CSV file - the file type is IMPORTANT
#8) Click OK to all text boxes and close excel
#9) Repeat for all desired samples
...
#9) Optional: Plots data as heatmap - this requires the file UsefulFunctions.R (see below)
#10) Optional: Performs principal component analysis

#From the UsefulFunctions.R script, you MUST load the following functions into your workspace:
#1) zscore()
...
#V6.2 - Allows user to specify parametric (student's t-test) OR nonparametric (Wilcoxon rank sum) statistical test
#V7 - Regression, nonparametric statistics, and Bayesian/Gibbs/bootstrapping functionality added.
#V8 - Enrichment functionality added. 
...
wd2 = "C:/Users/jeshima/Documents/Smith Lab/Spring 2023/BMC Microbiology Paper/Data/EF_G2"
setwd(wd2)
...
wd = "C:/Users/jeshima/Documents/Smith Lab/Spring 2023/BMC Microbiology Paper/Data/EF_G2/RTs"
setwd(wd)
RTind = read.csv("EF_RTs.csv")
...
Remove_LowRT = 4.5 #Not needed, if ClearNoise = False
Remove_HighRT = 5.0 #Not needed, if ClearNoise = False
...
OD600_pos = c(0.109,0.084,0.127)
OD600_neg = c(0.093,0.110,0.068)
...
AllowUnadjusted = T #Should the unadjusted p-values be considered/used? This might be useful to run if VOCs are weakly significant.
sigthresh = 0.1 #Significance threshold
addpvalues = F #Should p-values be shown for VOCs that do NOT pass the significance threshold?
...
negcolor = "purple"
ctrcolor = "gray50"
excolor = "orange" #This is only needed if 4 conditions are provided
lowerlim = -0.5 #manually adjust lower limit
...
pcacolorpos = "darkred"
pcacolorneg = "darkblue"
pcacolorctr = "green"
pcacolorex = "purple" #This is only needed if 4 conditions are provided
...
for(i in 1:length(files2)){
  ...
  if(combinebroadpeaks == T){
    ...
    for(z in 1:nrow(RTind)){
      cleandata$RT[z] = mean(RTind$Start[z],RTind$Stop[z])
    }
    ...
    for(j in 1:nrow(RTind)){
      ...
      if(j == 1){
        filterfile = myfile[-finalindex,]
      }else{
        filterfile = filterfile[-finalindex,]
      }
...
PosData = cbind(PosData,PosAb,PosRTs)
if(maxind == 1){
  PosData = PosData[,-1]
}else if(maxind == 2){
  PosData = PosData[,-2]
...
NegData = cbind(NegData,NegAb,NegRTs)
if(maxind == 1){
  NegData = NegData[,-1]
}else if(maxind == 2){
  NegData = NegData[,-2]
...
if(length(conditions) == 3){
  ...
  CtrData = cbind(CtrData,CtrAb,CtrRTs)
  if(maxind == 1){
    CtrData = CtrData[,-1]
  }else if(maxind == 2){
    CtrData = CtrData[,-2]
...
if(length(conditions) == 4){
  ...
  ExData = cbind(ExData,ExAb,ExRTs)
  if(maxind == 1){
    ExData = ExData[,-1]
  }else if(maxind == 2){
    ExData = ExData[,-2]
...
for(i in 1:length(fillPosfiles)){
  ...
  for(j in 1:nrow(data)){
    for(k in 1:nrow(PosData)){
      ...
      if(data$RT[j] > PosData$RT[k] - RTtol && data$RT[j] < PosData$RT[k] + RTtol ){
        
        PosData[k,i] = data$Abundance[j]
...
for(i in 1:length(fillNegfiles)){
  ...
  for(j in 1:nrow(data)){
    for(k in 1:nrow(NegData)){
      ...
      if(data$RT[j] > NegData$RT[k] - RTtol && data$RT[j] < NegData$RT[k] + RTtol ){
        
        NegData[k,i] = data$Abundance[j]
...
for(i in 1:length(fillDFs)){
  ...
  if(length(conditions) == 2){
    ...
    }else if(substr(fillDFs[i],1,3) == conditions[4]){
...
if(ClearNoise == T){
  ...
  for(i in 1:nrow(FinalData)){
    ...
    if(tmpdata$RT[i] <= Remove_HighRT && tmpdata$RT[i] >= Remove_LowRT){
      manualrem[count] = i
      count = count+1
    }
...
if(PlotSigOnly == T){
  ...
  if(length(Cind) >= 3){ 
    ...
    for(i in 1:nrow(plotdata)){
      ...
      if(length(conditions) == 2){
        df$Condition = c("+L-Dopa","-L-Dopa")
      }else if(length(conditions) == 3){
        df$Condition = c("+L-Dopa","-L-Dopa","BHI Control")
      }
      ...
      if(length(conditions) == 2){
        df$Condition = factor(df$Condition,levels=c("-L-Dopa","+L-Dopa"))
        colors = c(negcolor,poscolor)
      }else if(length(conditions) == 3){
        df$Condition = factor(df$Condition,levels=c("BHI Control","-L-Dopa","+L-Dopa"))
    ...
    if(length(Cind) >= 3){ 
      ...
      for(i in 1:nrow(plotdata)){
        ...
        df = data.frame(matrix(NA,length(conditions),3))
        colnames(df) = c("Condition","Average","SE")
        df$Condition = c("+L-Dopa","-L-Dopa","BHI Control","AFMT Inhibition")
        ...
        #Dataframe for plotting
        df$Condition = factor(df$Condition,levels=c("BHI Control","-L-Dopa","+L-Dopa","AFMT Inhibition"))
        colors = c(ctrcolor,negcolor,poscolor,excolor)
        
        #BARPLOTHELP
        #Flip order so that it gives a specific order
        #df$Condition = factor(df$Condition,levels=c("+L-Dopa","AFMT Inhibition","-L-Dopa","BHI Control"))
      ...
      for(i in 1:nrow(plotdata)){
        ...
        if(length(Cind) >= 1){
          ...
          df = data.frame(matrix(NA,length(conditions),3))
          colnames(df) = c("Condition","Average","SE")
          df$Condition = c("+L-Dopa","-L-Dopa","AFMT Inhibition")
          ...
          #Dataframe for plotting
          df$Condition = factor(df$Condition,levels=c("-L-Dopa","+L-Dopa","AFMT Inhibition"))
...
if(RunPCA == T){
  ...
  if(length(conditions) <= 3){
    ...
    if(pca3d == T){
      ...
      legend(max1 + (0.2*max1),max2 + (0.55*max2),legend = c("Positive","Negative","Control","AFMT Inhibition"),col = c(pcacolorpos,pcacolorneg,pcacolorctr,pcacolorex),pch=20,pt.cex = 1.5)
    }else{
